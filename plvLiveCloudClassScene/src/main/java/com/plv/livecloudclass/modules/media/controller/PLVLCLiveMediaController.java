package com.plv.livecloudclass.modules.media.controller;
...
import com.plv.livecloudclass.modules.media.widget.PLVLCLiveMoreLayout;
import com.plv.livecloudclass.modules.media.widget.PLVLCPPTTurnPageLayout;
...
public class PLVLCLiveMediaController extends FrameLayout implements IPLVLCLiveMediaController, View.OnClickListener {
    ...
    private List<PLVDefinitionVO> definitionVOS;
    //码率弹窗布局
    ...
    private void initMoreLayout() {
        ...
        moreLayout.setOnBitrateSelectedListener(new PLVLCLiveMoreLayout.OnBitrateSelectedListener() {
            @Override
            public void onBitrateSelected(PLVDefinitionVO definitionVO, int pos) {
        ...
        moreLayout.setOnOnlyAudioSwitchListener(new PLVLCLiveMoreLayout.OnOnlyAudioSwitchListener() {
            ...
            public boolean onOnlyAudioSelect(boolean onlyAudio) {
                if (onlyAudio) {
                    livePlayerPresenter.changeMediaPlayMode(PLVMediaPlayMode.MODE_AUDIO);
                } else {
                    livePlayerPresenter.changeMediaPlayMode(PLVMediaPlayMode.MODE_VIDEO);
                }
                return true;
            }
        });
    ...
    // <editor-fold defaultstate="collapsed" desc="对外API - 实现IPLVLCLiveMediaController父接口IPLVMediaController定义的方法">
    @Override
    ...
    private void updateMoreLayout() {
        ...
        Pair<List<PLVDefinitionVO>, Integer> listIntegerPair = new Pair<>(livePlayerPresenter.getBitrateVO(), livePlayerPresenter.getBitratePos());
    ...
    private void showBitrateChangeView() {
        if (definitionVOS == null
                ...
                || livePlayerPresenter.getMediaPlayMode() == PLVMediaPlayMode.MODE_AUDIO) {
            return;
        }
        ...
        PLVDefinitionVO definitionVO = definitionVOS.get(Math.max(0, currentBitratePos + 1));//超清，高清，标清
