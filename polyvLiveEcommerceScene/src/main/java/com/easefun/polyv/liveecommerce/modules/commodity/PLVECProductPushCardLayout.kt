...
class PLVECProductPushCardLayout @JvmOverloads constructor(
    ...
    fun init(liveRoomDataManager: IPLVLiveRoomDataManager) {
        ...
        val productHotEffectTips = PLVChannelFeatureManager.onChannel(liveRoomDataManager.config.channelId).get(PLVChannelFeature.LIVE_PRODUCT_HOT_EFFECT_TIPS) ?: ProductHotEffectBean().apply {
            normalProductTips = getString(R.string.plvec_product_push_hot_effect_text_normal)
            financeProductTips = getString(R.string.plvec_product_push_hot_effect_text_finance)
            jobProductTips = getString(R.string.plvec_product_push_hot_effect_text_job)
        }
    ...
    private fun enterCommodity(product: PLVProductContentBean): Boolean {
        ...
        PLVECCommodityDetailActivity.start(context, link, liveRoomDataManager)
        return true
    }
...
private class ImageProductCard(context: Context) : AbsCardLayout(context) {
    ...
    private val productPushPriceStrickOutTv by lazy { findViewById<TextView>(R.id.plvec_product_push_strick_out_price_tv) }
    private val productPushPriceStrickOutTv2 by lazy { findViewById<TextView>(R.id.plvec_product_push_strick_out_price_tv_2) }
    ...
    private val productPushSeckillLy by lazy { findViewById<ViewGroup>(R.id.plvec_product_push_seckill_ly) }
    private val productPushSeckillTv by lazy { findViewById<TextView>(R.id.plvec_product_push_seckill_tv) }
