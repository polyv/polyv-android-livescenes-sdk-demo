...
import static com.easefun.polyv.livecommon.ui.widget.seekbar.PLVSeekBar.INDICATOR_ALWAYS_HIDE;
import static com.easefun.polyv.livecommon.ui.widget.seekbar.PLVSeekBar.INDICATOR_ALWAYS_SHOW;
...
public class PLVRangeSeekBar extends View {
    ...
    //从中间刻度 往两边去绘制，只有stepsAutoBonding = true 和 多steps 才会生效
    private boolean stepsMidtoSideDrawMod;
    ...
    PLVSeekBar leftSB;
    PLVSeekBar rightSB;
    PLVSeekBar currTouchSB;
    Bitmap progressBitmap;
    Bitmap progressDefaultBitmap;
    List<Bitmap> stepsBitmaps = new ArrayList<>();
    private int progressPaddingRight;
    private PLVSeekBarOnRangeChangedListener callback;

    public PLVRangeSeekBar(Context context) {
        this(context, null);
    }

    public PLVRangeSeekBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttrs(attrs);
        initPaint();
        initSeekBar(attrs);
    ...
    private void initProgressBitmap() {
        if (progressBitmap == null) {
            progressBitmap = PLVSeekbarUtils.drawableToBitmap(getContext(), progressWidth, progressHeight, progressDrawableId);
        }
        if (progressDefaultBitmap == null) {
            progressDefaultBitmap = PLVSeekbarUtils.drawableToBitmap(getContext(), progressWidth, progressHeight, progressDefaultDrawableId);
        }
    }

    private boolean verifyStepsMode() {
        if (steps < 1 || stepsHeight <= 0 || stepsWidth <= 0) {
            return false;
        }
        return true;
    }
    ...
    private void initStepsBitmap() {
        ...
        if (stepsBitmaps.isEmpty()) {
            Bitmap bitmap = PLVSeekbarUtils.drawableToBitmap(getContext(), (int) stepsWidth, (int) stepsHeight, stepsDrawableId);
    ...
    private void initSeekBar(AttributeSet attrs) {
        leftSB = new PLVSeekBar(this, attrs, true);
        rightSB = new PLVSeekBar(this, attrs, false);
    ...
    private void initAttrs(AttributeSet attrs) {
        try {
            TypedArray t = getContext().obtainStyledAttributes(attrs, R.styleable.PLVRangeSeekBar);
            seekBarMode = t.getInt(R.styleable.PLVRangeSeekBar_plv_rsb_mode, SEEKBAR_MODE_RANGE);
            minProgress = t.getFloat(R.styleable.PLVRangeSeekBar_plv_rsb_min, 0);
            maxProgress = t.getFloat(R.styleable.PLVRangeSeekBar_plv_rsb_max, 100);
            minInterval = t.getFloat(R.styleable.PLVRangeSeekBar_plv_rsb_min_interval, 0);
            gravity = t.getInt(R.styleable.PLVRangeSeekBar_plv_rsb_gravity, Gravity.TOP);
            progressColor = t.getColor(R.styleable.PLVRangeSeekBar_plv_rsb_progress_color, 0xFF4BD962);
            progressRadius = (int) t.getDimension(R.styleable.PLVRangeSeekBar_plv_rsb_progress_radius, -1);
            progressDefaultColor = t.getColor(R.styleable.PLVRangeSeekBar_plv_rsb_progress_default_color, 0xFFD7D7D7);
            progressDrawableId = t.getResourceId(R.styleable.PLVRangeSeekBar_plv_rsb_progress_drawable, 0);
            progressDefaultDrawableId = t.getResourceId(R.styleable.PLVRangeSeekBar_plv_rsb_progress_drawable_default, 0);
            progressHeight = (int) t.getDimension(R.styleable.PLVRangeSeekBar_plv_rsb_progress_height, PLVSeekbarUtils.dp2px(2));
            tickMarkMode = t.getInt(R.styleable.PLVRangeSeekBar_plv_rsb_tick_mark_mode, TRICK_MARK_MODE_NUMBER);
            tickMarkGravity = t.getInt(R.styleable.PLVRangeSeekBar_plv_rsb_tick_mark_gravity, TICK_MARK_GRAVITY_CENTER);
            tickMarkLayoutGravity = t.getInt(R.styleable.PLVRangeSeekBar_plv_rsb_tick_mark_layout_gravity, Gravity.TOP);
            tickMarkTextArray = t.getTextArray(R.styleable.PLVRangeSeekBar_plv_rsb_tick_mark_text_array);
            tickMarkTextMargin = (int) t.getDimension(R.styleable.PLVRangeSeekBar_plv_rsb_tick_mark_text_margin, PLVSeekbarUtils.dp2px(7));
            tickMarkTextSize = (int) t.getDimension(R.styleable.PLVRangeSeekBar_plv_rsb_tick_mark_text_size, PLVSeekbarUtils.dp2px(12));
            tickMarkTextColor = t.getColor(R.styleable.PLVRangeSeekBar_plv_rsb_tick_mark_text_color, progressDefaultColor);
            tickMarkInRangeTextColor = t.getColor(R.styleable.PLVRangeSeekBar_plv_rsb_tick_mark_in_range_text_color, progressColor);
            steps = t.getInt(R.styleable.PLVRangeSeekBar_plv_rsb_steps, 0);
            stepsColor = t.getColor(R.styleable.PLVRangeSeekBar_plv_rsb_step_color, 0xFF9d9d9d);
            stepsRadius = t.getDimension(R.styleable.PLVRangeSeekBar_plv_rsb_step_radius, 0);
            stepsWidth = t.getDimension(R.styleable.PLVRangeSeekBar_plv_rsb_step_width, 0);
            stepsHeight = t.getDimension(R.styleable.PLVRangeSeekBar_plv_rsb_step_height, 0);
            stepsDrawableId = t.getResourceId(R.styleable.PLVRangeSeekBar_plv_rsb_step_drawable, 0);
            stepsAutoBonding = t.getBoolean(R.styleable.PLVRangeSeekBar_plv_rsb_step_auto_bonding, true);

            stepsMidtoSideDrawMod = t.getBoolean(R.styleable.PLVRangeSeekBar_plv_rsb_mid_side_drawable_mode, false);
            t.recycle();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected void onMeasureProgress(int w, int h) {
        ...
        if (gravity == Gravity.TOP) {
            ...
            Log.d("TAG","=== paddingTop " + getPaddingTop());
    ...
    protected int getTickMarkRawHeight() {
        if (tickMarkTextArray != null && tickMarkTextArray.length > 0) {
            return tickMarkTextMargin + PLVSeekbarUtils.measureText(String.valueOf(tickMarkTextArray[0]), tickMarkTextSize).height() + 3;
        }
        return 0;
    }
        ...
        if (tickMarkTextArray != null) {
            ...
            for (int i = 0; i < tickMarkTextArray.length; i++) {
                ...
                if (tickMarkMode == TRICK_MARK_MODE_OTHER) {
                    ...
                    float num = PLVSeekbarUtils.parseFloat(text2Draw);
                    PLVSeekBarState[] states = getRangeSeekBarState();
                    if (PLVSeekbarUtils.compareFloat(num, states[0].value) != -1 && PLVSeekbarUtils.compareFloat(num, states[1].value) != 1 && (seekBarMode == SEEKBAR_MODE_RANGE)) {
    ...
    protected void onDrawProgressBar(Canvas canvas, Paint paint) {
        ...
        if (PLVSeekbarUtils.verifyBitmap(progressDefaultBitmap)) {
        ...
        if (seekBarMode == SEEKBAR_MODE_RANGE) {
            ...
            if(verifyStepsMode() && stepsAutoBonding && stepsMidtoSideDrawMod){
                //从中间往两边取绘制
                progressDstRect.top = getProgressTop();
                progressDstRect.bottom = getProgressBottom();
                ...
                int centerStpes = steps / 2 + steps % 2;
                float center = getProgressLeft() + centerStpes * stepMarks - stepsWidth / 2f;
                if(leftSB.currPercent >= range / 2) {
                    progressDstRect.left = leftSB.left + center;
                    progressDstRect.right = leftSB.left + leftSB.getThumbScaleWidth() / 2f + progressWidth * (leftSB.currPercent - 0.5f);
                } else {
                    progressDstRect.left = leftSB.left + leftSB.getThumbScaleWidth() / 2f + progressWidth * leftSB.currPercent;
                    progressDstRect.right = leftSB.left + center;
                }
            } else {
                progressDstRect.top = getProgressTop();
        ...
        if (PLVSeekbarUtils.verifyBitmap(progressBitmap)) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (verifyStepsMode() && stepsAutoBonding) {
                    ...
                    if(steps > 1){
                        if(callback != null){
                            callback.onRangeChangeStep(stepSelected);
                        }
                    }
                }
...
//        PLVSavedState ss = new PLVSavedState(superState);
//        ss.minValue = minProgress;
//        ss.maxValue = maxProgress;
//        ss.rangeInterval = minInterval;
//        PLVSeekBarState[] results = getRangeSeekBarState();
...
//        try {
//            PLVSavedState ss = (PLVSavedState) state;
...
//    }
    ...
    public void setOnRangeChangedListener(PLVSeekBarOnRangeChangedListener listener) {
        callback = listener;
    }
    ...
    public void setProgress(float leftValue, float rightValue) {
        ...
        if (verifyStepsMode() && stepsAutoBonding) {
            float stepPercent = 1.0f / steps;
            int stepSelected = new BigDecimal(leftSB.currPercent / stepPercent).setScale(0, RoundingMode.HALF_UP).intValue();
            currTouchSB = leftSB;
            currTouchSB.slide(stepSelected * stepPercent);
        }
    }
    ...
    /**
     * @return the two seekBar state
     */
    public PLVSeekBarState[] getRangeSeekBarState() {
        PLVSeekBarState leftSeekBarState = new PLVSeekBarState();
        leftSeekBarState.value = leftSB.getProgress();

        leftSeekBarState.indicatorText = String.valueOf(leftSeekBarState.value);
        if (PLVSeekbarUtils.compareFloat(leftSeekBarState.value, minProgress) == 0) {
            leftSeekBarState.isMin = true;
        } else if (PLVSeekbarUtils.compareFloat(leftSeekBarState.value, maxProgress) == 0) {
            leftSeekBarState.isMax = true;
        }

        PLVSeekBarState rightSeekBarState = new PLVSeekBarState();
        if (seekBarMode == SEEKBAR_MODE_RANGE) {
            rightSeekBarState.value = rightSB.getProgress();
            rightSeekBarState.indicatorText = String.valueOf(rightSeekBarState.value);
            if (PLVSeekbarUtils.compareFloat(rightSB.currPercent, minProgress) == 0) {
                rightSeekBarState.isMin = true;
            } else if (PLVSeekbarUtils.compareFloat(rightSB.currPercent, maxProgress) == 0) {
                rightSeekBarState.isMax = true;
            }
        }

        return new PLVSeekBarState[]{leftSeekBarState, rightSeekBarState};
    }
    ...
    /**
     * 获取当前步数，如果不是多步数和开启自动粘合则 都返回0
     * @return
     */
    public int getCurrentStep(){
        if (verifyStepsMode() && stepsAutoBonding) {
            float percent = currTouchSB.currPercent;
            float stepPercent = 1.0f / steps;
            int stepSelected = new BigDecimal(percent / stepPercent).setScale(0, RoundingMode.HALF_UP).intValue();
            return stepSelected;
        }
     ...
     */
    public PLVSeekBar getLeftSeekBar() {
        return leftSB;
    }

    public PLVSeekBar getRightSeekBar() {
        return rightSB;
    }
    ...
    public void setMidToSideDrawMode(boolean isOpen){
        this.stepsMidtoSideDrawMod = isOpen;
    }

    public boolean getMidToSideDrawMode(){
        return stepsMidtoSideDrawMod;
    }
    ...
    public void setStepsDrawable(List<Integer> stepsDrawableIds) {
        ...
        for (int i = 0; i < stepsDrawableIds.size(); i++) {
            stepsBitmaps.add(PLVSeekbarUtils.drawableToBitmap(getContext(), (int) stepsWidth, (int) stepsHeight, stepsDrawableIds.get(i)));
        }
