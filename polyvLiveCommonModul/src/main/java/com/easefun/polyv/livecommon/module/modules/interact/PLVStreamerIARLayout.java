...
import com.plv.livescenes.feature.interact.PLVStreamerIARWebView;
...
public class PLVStreamerIARLayout extends FrameLayout implements IPLVStreamerIARLayout {
    // <editor-fold defaultstate="collapsed" desc="变量">
    private static final String TAG = PLVStreamerIARLayout.class.getSimpleName();
    private IPLVLiveRoomDataManager liveRoomDataManager;
    private PLVStreamerIARWebView plvStreamerIARWeb;

    private static final String EVENT_DOWNLOADSIGNINRECORD = "downloadSignInRecord";
    private static final String EVENT_DOWNLOAD_LUCKY_BAG_RECORD = "downloadLuckyBagRecord";
    ...
    public PLVStreamerIARLayout(@NonNull Context context) {
        this(context, null);
    }

    public PLVStreamerIARLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PLVStreamerIARLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView() {
        LayoutInflater.from(getContext()).inflate(R.layout.plv_streamer_iar_layout, this, true);
        plvStreamerIARWeb = findViewById(R.id.plv_streamer_iar_web);
        setVisibility(View.INVISIBLE);

        for (final String event : JS_HANDLER) {
            plvStreamerIARWeb.registerHandler(event, new BridgeHandler() {
                @Override
    ...
    public void init(IPLVLiveRoomDataManager liveRoomDataManager) {
        ...
        plvStreamerIARWeb.setLang(PLVLanguageUtil.isENLanguage() ? PLVStreamerInteractWebView.LANG_EN : PLVStreamerInteractWebView.LANG_ZH);
        plvStreamerIARWeb.loadWeb();

    }

    @Override
    public void showLuckyBag() {
        String data = "{\"event\" : \"SHOW_LUCKY_BAG\"}";
        plvStreamerIARWeb.sendMsgToJs(PLVInteractJSBridgeEventConst.V2_APP_CALL_WEB_VIEW_EVENT, data, new CallBackFunction() {
            @Override
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        if (plvStreamerIARWeb != null) {
            plvStreamerIARWeb.onActivityResult(requestCode, resultCode, intent);
        }
    }
    ...
    public void destroy() {
        if (plvStreamerIARWeb != null) {
            plvStreamerIARWeb.removeAllViews();
            ViewParent viewParent = plvStreamerIARWeb.getParent();
            if (viewParent instanceof ViewGroup) {
                ViewGroup viewGroup = (ViewGroup) viewParent;
                viewGroup.removeView(plvStreamerIARWeb);
            }
            plvStreamerIARWeb.destroy();
            plvStreamerIARWeb = null;
        }
    }
    // </editor-fold >
           ...
           JSONObject value = PLVJsonUtils.getObject(jsonObject,"value", null);
            switch (event) {
                case EVENT_DOWNLOADSIGNINRECORD:
                    processorDownloadRecord(value, "PLVSignInRecord", EVENT_DOWNLOADSIGNINRECORD);
                    break;
                case EVENT_DOWNLOAD_LUCKY_BAG_RECORD:
                    processorDownloadRecord(value, "PLVLuckyBagRecord", EVENT_DOWNLOAD_LUCKY_BAG_RECORD);
                    break;
                default:
            }
    ...
    private void processorDownloadRecord(JSONObject value, String subDir, String event) {
        if (value != null) {
            PLVCommonLog.d(TAG, "processorDownloadRecord is: " + value.toString());
            final String url = PLVJsonUtils.getString(value, "downloadURL", "");
            final String fileName = PLVJsonUtils.getString(value, "fileName", "");
            ...
            PLVStoragePermissionCompat
                    .start((Activity) getContext(), new PLVOnPermissionCallback() {
                        ...
                        public void onAllGranted() {
                            final String savePath = PLVSDCardUtils.createPath(getContext(), subDir);
                            downloadRecord(url, savePath, fileName);
                        }
    ...
    private void downloadRecord(String url, String savePath, String fileName) {

        PLVInteractFileDownloadManager.getInstance().requestDownLoadFile(url, savePath, fileName, new IPLVInteractFileDownloadListener() {
            @Override
