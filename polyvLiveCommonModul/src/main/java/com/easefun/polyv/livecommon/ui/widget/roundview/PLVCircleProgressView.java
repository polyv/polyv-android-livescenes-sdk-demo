...
public class PLVCircleProgressView extends ProgressBar {
    ...
    private int mTextSize = PolyvDpUtils.sp2px(getContext(), 14); // 进度值字体大小
    ...
    private int mInnerPadding = PolyvDpUtils.dp2px(getContext(), 1); // 内部圆与外部圆间距
    ...
    private int mOuterSize = PolyvDpUtils.dp2px(getContext(), 1); // 外层圆环宽度
    ...
    private void obtainAttributes(AttributeSet attrs) {
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.PLVCircleProgressView);
        mProgressStyle = ta.getInt(R.styleable.PLVCircleProgressView_plv_cpv_progressStyle, ProgressStyle.NORMAL);
        // 获取三种风格通用的属性
        mNormalBarSize = (int) ta.getDimension(R.styleable.PLVCircleProgressView_plv_cpv_progressNormalSize, mNormalBarSize);
        mNormalBarColor = ta.getColor(R.styleable.PLVCircleProgressView_plv_cpv_progressNormalColor, mNormalBarColor);

        mReachBarSize = (int) ta.getDimension(R.styleable.PLVCircleProgressView_plv_cpv_progressReachSize, mReachBarSize);
        mReachBarColor = ta.getColor(R.styleable.PLVCircleProgressView_plv_cpv_progressReachColor, mReachBarColor);

        mTextSize = (int) ta.getDimension(R.styleable.PLVCircleProgressView_plv_cpv_progressTextSize, mTextSize);
        mTextColor = ta.getColor(R.styleable.PLVCircleProgressView_plv_cpv_progressTextColor, mTextColor);
        mTextSkewX = ta.getDimension(R.styleable.PLVCircleProgressView_plv_cpv_progressTextSkewX, 0);
        if (ta.hasValue(R.styleable.PLVCircleProgressView_plv_cpv_progressTextSuffix)) {
            mTextSuffix = ta.getString(R.styleable.PLVCircleProgressView_plv_cpv_progressTextSuffix);
        }
        if (ta.hasValue(R.styleable.PLVCircleProgressView_plv_cpv_progressTextPrefix)) {
            mTextPrefix = ta.getString(R.styleable.PLVCircleProgressView_plv_cpv_progressTextPrefix);
        }
        mTextVisible = ta.getBoolean(R.styleable.PLVCircleProgressView_plv_cpv_progressTextVisible, mTextVisible);

        mRadius = (int) ta.getDimension(R.styleable.PLVCircleProgressView_plv_cpv_radius, mRadius);
        ...
        switch (mProgressStyle) {
            ...
            case ProgressStyle.FILL_IN_ARC:
                mStartArc = ta.getInt(R.styleable.PLVCircleProgressView_plv_cpv_progressStartArc, 0) + 270;
                mInnerPadding = (int) ta.getDimension(R.styleable.PLVCircleProgressView_plv_cpv_innerPadding, mInnerPadding);
                mOuterColor = ta.getColor(R.styleable.PLVCircleProgressView_plv_cpv_outerColor, mReachBarColor);
                mOuterSize = (int) ta.getDimension(R.styleable.PLVCircleProgressView_plv_cpv_outerSize, mOuterSize);
                mReachBarSize = 0;// 将画笔大小重置为0
                mNormalBarSize = 0;
                if (!ta.hasValue(R.styleable.PLVCircleProgressView_plv_cpv_progressNormalColor)) {
            ...
            case ProgressStyle.NORMAL:
                mReachCapRound = ta.getBoolean(R.styleable.PLVCircleProgressView_plv_cpv_reachCapRound, true);
                mStartArc = ta.getInt(R.styleable.PLVCircleProgressView_plv_cpv_progressStartArc, 0) + 270;
                if (ta.hasValue(R.styleable.PLVCircleProgressView_plv_cpv_innerBackgroundColor)) {
                    mInnerBackgroundColor = ta.getColor(R.styleable.PLVCircleProgressView_plv_cpv_innerBackgroundColor, Color.argb(0, 0, 0, 0));
    ...
    public void setReachBarSize(int reachBarSize) {
        mReachBarSize = PolyvDpUtils.dp2px(getContext(), reachBarSize);
        invalidate();
    }
    ...
    public void setNormalBarSize(int normalBarSize) {
        mNormalBarSize = PolyvDpUtils.dp2px(getContext(), normalBarSize);
        invalidate();
    }
    ...
    public void setTextSize(int textSize) {
        mTextSize = PolyvDpUtils.sp2px(getContext(), textSize);
        invalidate();
    }
    ...
    public void setRadius(int radius) {
        mRadius = PolyvDpUtils.dp2px(getContext(), radius);
        invalidate();
    }
    ...
    public void setInnerPadding(int innerPadding) {
        mInnerPadding = PolyvDpUtils.dp2px(getContext(), innerPadding);
        int mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding;
        rectInner = new RectF(-mInnerRadius, -mInnerRadius, mInnerRadius, mInnerRadius);
        invalidate();
    }
    ...
    public void setOuterSize(int outerSize) {
        mOuterSize = PolyvDpUtils.dp2px(getContext(), outerSize);
        invalidate();
    }
