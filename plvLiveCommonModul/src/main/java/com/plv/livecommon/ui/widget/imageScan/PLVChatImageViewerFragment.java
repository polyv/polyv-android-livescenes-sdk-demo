package com.plv.livecommon.ui.widget.imageScan;
...
import com.plv.livecommon.module.utils.imageloader.PLVUrlTag;
...
public class PLVChatImageViewerFragment extends PLVBaseFragment {
    ...
    public static int toImgList(List<PLVBaseViewData> dataList, PLVBaseViewData<PLVBaseEvent> selData, List<PLVUrlTag> imgTagList) {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            } else if (baseEvent instanceof PLVSendLocalImgEvent) {
                imgUrl = ((PLVSendLocalImgEvent) baseEvent).getImageFilePath();
                if (((PLVSendLocalImgEvent) baseEvent).getObj2() instanceof PLVUrlTag) {
                    urlTag = (PLVUrlTag) ((PLVSendLocalImgEvent) baseEvent).getObj2();
                } else {
                    urlTag = new PLVUrlTag(imgUrl, baseEvent);
                    ((PLVSendLocalImgEvent) baseEvent).setObj2(urlTag);
                }
            } else {
                continue;
            }
