...
// </editor-fold>
        ...
        switch (event) {
            ...
            case PLVSocketEvent.GET_TEST_QUESTION_CONTENT:
                delay(new Runnable() {
                    @Override
                    public void run() {
                        PLVQuestionSResult questionSResult = PLVGsonUtil.fromJson(PLVQuestionSResult.class, msg);
                        if (questionSResult == null) {
                            return;
                        }
                        curQuestionId = questionSResult.getQuestionId();
            ...
            case PLVSocketEvent.GET_TEST_QUESTION_RESULT:
                delay(new Runnable() {
                    ...
                    public void run() {
                        ...
                        PLVQuestionResultJsVO questionResultJsVO;
                        if (questionVO == null) {
                            questionResultJsVO = new PLVQuestionResultJsVO("", msg);
                        } else {
                            questionResultJsVO = new PLVQuestionResultJsVO(questionVO.getAnswerId(), msg);
                        }

                        sendMsgToJs(PLVInteractJSBridgeEventConst.ANSWER_SHEET_RESULT, questionResultJsVO.toString(), new CallBackFunction() {
                            @Override
                            public void onCallBack(String data) {
    ...
    private void saveSelectedOption(String data) {
        PLVJSQuestionVO jsQuestionVO = PLVGsonUtil.fromJson(PLVJSQuestionVO.class, data);
        PLVCommonLog.d(TAG, "receive result answer " + data);
        if (jsQuestionVO != null && !TextUtils.isEmpty(jsQuestionVO.getQuestionId())) {
            sendResultToServer(jsQuestionVO);
            PLVCommonLog.d(TAG, "save answer :" + jsQuestionVO.getQuestionId());
            questions.put(jsQuestionVO.getQuestionId(), jsQuestionVO);
        }
    }
...
// </editor-fold>
    ...
    private void sendResultToServer(PLVJSQuestionVO jsQuestionVO) {
        PLVQuestionSocketVO socketVO = new PLVQuestionSocketVO(jsQuestionVO.getAnswerId(), viewerName, jsQuestionVO.getQuestionId(), channelId, viewerId);
