...
import com.plv.livecommon.module.modules.document.model.PLVDocumentNetRepo;
...
public class PLVDocumentNetPresenter {
    ...
    private Observer<PLVStatefulData<PLVPPTInfo>> pptInfoObserver;
    ...
    private void observePptInfo() {
        plvDocumentRepository.getPptInfoLiveData().observeForever(pptInfoObserver = new Observer<PLVStatefulData<PLVPPTInfo>>() {
            @Override
            public void onChanged(@Nullable PLVStatefulData<PLVPPTInfo> plvspptInfo) {
    ...
    private void observePptOnDeleteResponse() {
        plvDocumentRepository.getPptOnDeleteResponseLiveData().observeForever(pptOnDeleteObserver = new Observer<PLVStatefulData<PLVPPTInfo.DataBean.ContentsBean>>() {
            @Override
    ...
    public void uploadFile(Context context, File uploadFile, final String convertType, final OnPLVDocumentUploadListener listener) {
        ...
        plvDocumentRepository.uploadPptFile(context, uploadFile, convertType, new OnPLVDocumentUploadListener() {
            @Override
    ...
    public void removeUploadCache(int autoId) {
        ...
        PLVPPTInfo.DataBean.ContentsBean contentsBean = getPptContentsBeanFromAutoId(autoId);
    ...
    private PLVPPTInfo.DataBean.ContentsBean getPptContentsBeanFromAutoId(int autoId) {
        ...
        PLVPPTInfo pptInfo = plvDocumentRepository.getCachePptCoverList();
        if (pptInfo == null || pptInfo.getData() == null || pptInfo.getData().getContents() == null) {
            Log.w(TAG, "cache ppt cover list is null.");
            return null;
        }
        List<PLVPPTInfo.DataBean.ContentsBean> contentsBeans = pptInfo.getData().getContents();
        for (PLVPPTInfo.DataBean.ContentsBean contentsBean : contentsBeans) {
