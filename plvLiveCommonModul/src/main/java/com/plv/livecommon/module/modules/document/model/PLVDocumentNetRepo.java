...
public class PLVDocumentNetRepo {
    ...
    // 数据 - PPT文档列表变更
    protected final MutableLiveData<PLVStatefulData<PLVPPTInfo>> plvsPptInfoLiveData = new MutableLiveData<>();
    // 事件 - PPT文档删除响应回调
    ...
    private void initDocumentUploadManager() {
        documentUploadManager = PLVDocumentUploadManagerFactory.createDocumentUploadManager();
    ...
    public void requestPptCoverList(boolean forceRefresh) {
        ...
        PLVDocumentDataManager.getDocumentList(new PLVrResponseCallback<PLVPPTInfo>() {
            ...
            public void onSuccess(PLVPPTInfo plvspptInfo) {
                cachePptInfo = PLVReflectionUtil.copyField(plvspptInfo, new PLVPPTInfo());
                plvsPptInfoLiveData.postValue(PLVStatefulData.success(cachePptInfo));
            }
    ...
    public void deleteDocument(final int autoId) {
        ...
        PLVPPTInfo.DataBean.ContentsBean deleteBean = null;
        // 根据autoId遍历获取需要删除的文档vo
        for (PLVPPTInfo.DataBean.ContentsBean bean : cachePptInfo.getData().getContents()) {
        ...
        final PLVPPTInfo.DataBean.ContentsBean finalDeleteBean = deleteBean;
        PLVDocumentDataManager.delDocument(deleteBean, new PLVrResponseCallback<ResponseBody>() {
            ...
            public void onError(Throwable e) {
                pptOnDeleteResponseLiveData.postValue(PLVStatefulData.<PLVPPTInfo.DataBean.ContentsBean>error(e.getMessage(), e));
            }
        });
    }
