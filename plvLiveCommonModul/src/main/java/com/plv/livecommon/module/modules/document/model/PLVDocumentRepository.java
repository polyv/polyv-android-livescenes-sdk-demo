package com.plv.livecommon.module.modules.document.model;
...
public class PLVDocumentRepository {
    ...
    // WebProcessor 弱引用
    private WeakReference<PLVDocumentWebProcessor> documentWebProcessorWeakReference;

    private PLVSocketMessageObserver.OnMessageListener onMessageListener;

    // PPT文档上传管理器
    @Nullable
    private IPLVDocumentUploadManager documentUploadManager;
    private IPLVLiveRoomDataManager liveRoomDataManager;

    // 数据 - PPT文档列表变更
    private final MutableLiveData<PLVStatefulData<PLVPPTInfo>> plvsPptInfoLiveData = new MutableLiveData<>();
    // 数据 - PPT页面列表变更
    private final MutableLiveData<PLVStatefulData<PLVPPTJsModel>> plvsPptJsModelLiveData = new MutableLiveData<>();
    // 事件 - PPT内容变更
    private final MutableLiveData<String> refreshPptMessageLiveData = new MutableLiveData<>();
    // 事件 - PPT页面状态变更
    private final MutableLiveData<PLVPPTStatus> plvsPptStatusLiveData = new MutableLiveData<>();
    // 事件 - PPT文本标注内容变更
    private final MutableLiveData<PLVPPTPaintStatus> plvsPptPaintStatusLiveData = new MutableLiveData<>();

    /**
     * 缓存 PPT文档列表
     */
    private PLVPPTInfo cachePptInfo = null;
     ...
     * Value: ppt页面列表
     */
    private SparseArray<PLVPPTJsModel> cachePptJsModel = new SparseArray<>();

    // </editor-fold>

    // <editor-fold defaultstate="collapsed" desc="构造方法">

    public PLVDocumentRepository(PLVDocumentWebProcessor documentWebProcessor) {
    ...
    private void initWebProcessor() {
        ...
        webProcessor.registerJSHandler(new PLVDocumentWebProcessor.CloudClassJSCallback() {
            @Override
            ...
            @Override
            public void getPPTImagesList(PLVPPTJsModel jsModel) {
            ...
            @Override
            public void getPPTChangeStatus(PLVPPTStatus plvspptStatus) {
                super.getPPTChangeStatus(plvspptStatus);
                plvsPptStatusLiveData.postValue(plvspptStatus);
            }

            @Override
            public void getEditContent(PLVPPTPaintStatus content) {
     ...
     */
    @Nullable
    public PLVPPTInfo getCachePptCoverList() {
        return cachePptInfo;
    }
    ...
    public void requestGetCachedPptPageList(int autoId) {
        PLVPPTJsModel jsModel = cachePptJsModel.get(autoId);
        if (jsModel != null) {
            plvsPptJsModelLiveData.postValue(PLVStatefulData.success(jsModel));
        } else {
            plvsPptJsModelLiveData.postValue(PLVStatefulData.<PLVPPTJsModel>error("没有缓存对应的文档列表，请先打开该PPT文档"));
        }
    }
     ...
     * @param file     需要上传的文件
     * @param type     转码类型 {@link com.plv.livescenes.upload.PLVDocumentUploadConstant.PPTConvertType}
     * @param listener 上传监听回调
     */
    public void uploadPptFile(final Context context, final File file, final String type, final OnPLVDocumentUploadListener listener) {
    ...
    public LiveData<PLVStatefulData<PLVPPTInfo>> getPptInfoLiveData() {
        return PLVDocumentNetRepo.getInstance().getPptInfoLiveData();
    }

    public LiveData<PLVStatefulData<PLVPPTJsModel>> getPptJsModelLiveData() {
        return plvsPptJsModelLiveData;
    }

    public LiveData<PLVPPTStatus> getPptStatusLiveData() {
        return plvsPptStatusLiveData;
    }

    public LiveData<PLVPPTPaintStatus> getPptPaintStatusLiveData() {
