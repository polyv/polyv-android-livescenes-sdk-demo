...
public class PLVInteractQuestionnaire extends PLVInteractAppAbs {
    ...
    protected void processSocketMsg(String msg, String event) {
        switch (event) {
            //开始问卷调查
            case PLVSocketEvent.START_QUESTIONNAIRE:
                notifyShow();
                ...
                break;
            //停止问卷调查
            case PLVSocketEvent.STOP_QUESTIONNAIRE:
                ...
                break;
            //问卷调查结果
            case PLVSocketEvent.SEND_QUESTIONNAIRE_RESULT:
                ...
                break;
            //问卷统计相关数据事件
            case PLVSocketEvent.QUESTIONNAIRE_ACHIEVEMENT:
                notifyShow();
    ...
    protected void registerMsgReceiverFromJs(IPLVInteractJSBridge interactJSBridge) {
        interactJSBridge.registerMsgReceiverFromJs(PLVInteractJSBridgeEventConst.QUESTIONNAIRE_CHOOSE, new BridgeHandler() {
            ...
            public void handler(String data, CallBackFunction function) {
                ...
                List<PLVQuestionnaireSocketVO.AnswerBean> answerBeanList = new ArrayList<>();
                ...
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        answerBeanList.add(new PLVQuestionnaireSocketVO.AnswerBean(questionId, answer));
                    }
                    questionnaireId = jsonObject.optString("id");
                } catch (JSONException e) {
                    e.printStackTrace();
                }
                PLVQuestionnaireSocketVO socketVO = new PLVQuestionnaireSocketVO(questionnaireId, answerBeanList);
    ...
    private void sendResultToServer(PLVQuestionnaireSocketVO socketVO) {
        PLVCommonLog.d(TAG, "发送调查问卷答案");
        socketVO.setNick(viewerName);
        socketVO.setRoomId(channelId);
        socketVO.setUserId(viewerId);
        PLVChatroomManager.getInstance().sendInteractiveSocketMessage(Socket.EVENT_MESSAGE, socketVO, 3, PLVInteractiveCallbackVO.EVENT_QUESTIONNAIRE);
    }
    // </editor-fold>
}
