package com.plv.livecommon.module.modules.ppt.presenter;
...
import static com.plv.business.api.common.ppt.PLVLivePPTProcessor.CHAT_LOGIN;
import static com.plv.business.api.common.ppt.PLVLivePPTProcessor.SEND_SOCKET_EVENT;
import static com.plv.livescenes.log.ppt.PLVPPTElog.PPTEvent.PPT_RECEIVE_WEB_MESSAGE;
...
public class PLVPPTPresenter implements IPLVPPTContract.IPLVPPTPresenter {
        ...
        onMessageListener = new PLVSocketMessageObserver.OnMessageListener() {
            ...
            public void onMessage(String listenEvent, String event, String message) {
                if (PLVEventConstant.Ppt.ON_SLICE_START_EVENT.equals(event) ||
                    ...
                    //PPT和主屏幕切换位置
                    PLVPPTAuthentic pptAuthentic = PLVGsonUtil.fromJson(PLVPPTAuthentic.class, message);
                    if (pptAuthentic == null) {
                        return;
                    }
                    String status = pptAuthentic.getStatus();
                    if (PLVPPTAuthentic.PermissionStatus.OK.equals(status)) {
                        if (view != null) {
        ...
        followTeacherPptVideoLocationListener = new PLVSocketMessageObserver.OnMessageListener() {
            ...
            public void onMessage(String listenEvent, String event, String message) {
                if (PLVOnSliceIDEvent.EVENT.equals(event)) {
                    ...
                    PLVSocketWrapper.getInstance().getSocketObserver().removeOnMessageListener(this);
    ...
    public void sendPPTBrushMsg(String message) {
        PLVELogSender.send(PLVPPTElog.class, PPT_RECEIVE_WEB_MESSAGE, "event " + SEND_SOCKET_EVENT + "receive web message :" + message);
        //发送画笔事件
