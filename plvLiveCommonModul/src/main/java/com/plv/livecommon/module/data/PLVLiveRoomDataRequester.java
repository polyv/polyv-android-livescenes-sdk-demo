...
public class PLVLiveRoomDataRequester {
    ...
    void requestPageViewer(final IPLVNetRequestListener<Integer> listener) {
        ...
        pageViewerDisposable = PLVResponseExcutor.excuteDataBean(
                PLVApiManager.getPlvLiveStatusApi().increasePageViewer(PLVFormatUtils.parseInt(channelId), appId, ts, sign, PLVSignCreator.getSignatureMethod(), times),
    ...
    void requestChannelDetail(final IPLVNetRequestListener<PLVLiveClassDetailVO> listener) {
        ...
        channelDetailDisposable = PLVChatApiRequestHelper.getInstance().requestLiveClassDetailApi(channelId, appId, appSecret)
                ...
                .compose(new PLVRxBaseTransformer<PLVLiveClassDetailVO, PLVLiveClassDetailVO>())
    ...
    void requestChannelSwitch(final IPLVNetRequestListener<PLVChatFunctionSwitchVO> listener) {
        ...
        channelSwitchDisposable = PLVChatApiRequestHelper.getInstance().requestFunctionSwitch(channelId)
                .retryWhen(new PLVRxBaseRetryFunction(3, 3000))
                .compose(new PLVRxBaseTransformer<PLVChatFunctionSwitchVO, PLVChatFunctionSwitchVO>())
                .subscribe(new Consumer<PLVChatFunctionSwitchVO>() {
                    @Override
                    public void accept(PLVChatFunctionSwitchVO chatFunctionSwitchVO) throws Exception {
                        if (listener != null) {
                            listener.onSuccess(chatFunctionSwitchVO);
                        }
                    }
                }, new Consumer<Throwable>() {
                    @Override
    ...
    void requestProductList(int rank, final IPLVNetRequestListener<PLVCommodityVO> listener) {
        ...
        if (rank > -1) {
            commodityVOObservable = PLVApiManager.getPlvLiveStatusApi().getProductList(channelId, appId, timestamp, count, rank, sign, PLVSignCreator.getSignatureMethod());
        } else {
            commodityVOObservable = PLVApiManager.getPlvLiveStatusApi().getProductList(channelId, appId, timestamp, count, sign, PLVSignCreator.getSignatureMethod());
        }
    ...
    void requestLiveStatus(final IPLVNetRequestListener<PLVLiveRoomDataManager.LiveStatus> listener) {
        ...
        String channelId = getConfig().getChannelId();
        getLiveStatusDisposable = PLVResponseExcutor.excuteUndefinData(PLVApiManager.getPlvLiveStatusApi().getLiveStatusJson2(channelId),
                new PLVrResponseCallback<PLVLiveStatusVO>() {
                    ...
                    @Override
                    public void onFailure(PLVResponseBean<PLVLiveStatusVO> responseBean) {
    ...
    void requestUpdateChannelName(final IPLVNetRequestListener<String> listener) {
        ...
        updateChannelNameDisposable = PLVApiManager.getPlvLiveStatusApi().updateChannelName(channelId, ptime, channelName, sign)
