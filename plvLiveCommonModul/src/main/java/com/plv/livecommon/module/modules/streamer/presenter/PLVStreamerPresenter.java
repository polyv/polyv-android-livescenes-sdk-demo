...
import com.plv.livescenes.streamer.listener.IPLVStreamerOnLiveStatusChangeListener;
...
public class PLVStreamerPresenter implements IPLVStreamerContract.IStreamerPresenter {
    ...
    public void controlUserLinkMic(int position, boolean isAllowJoin) {
        ...
        if (isAllowJoin) {
            ...
            PLVLinkMicEventSender.getInstance().responseUserLinkMic(socketUserBean, new IPLVLinkMicEventSender.PLVMainCallAck() {
                @Override
    ...
    public void muteUserMedia(int position, final boolean isVideoType, final boolean isMute) {
        ...
        PLVLinkMicEventSender.getInstance().muteUserMedia(socketUserBean, sessionId, isVideoType, isMute, new IPLVLinkMicEventSender.PLVMainCallAck() {
            @Override
            public void onCall(Object... args) {
                if (linkMicItemDataBean != null) {
                    if (isVideoType) {
    ...
    private void pollLiveStatus() {
        streamerManager.listenLiveStatusChange(0, INTERVAL_TO_POLL_LIVE_STATUS, new IPLVStreamerOnLiveStatusChangeListener() {
            @Override
