package com.plv.livecommon.module.modules.streamer.presenter;
...
import com.plv.livecommon.module.modules.streamer.presenter.data.PLVStreamerData;
...
import com.plv.livescenes.model.PLVSListUsersVO;
import com.plv.livescenes.socket.PLVSocketWrapper;
import com.plv.livescenes.streamer.IPLVSStreamerManager;
...
import com.plv.livescenes.streamer.listener.IPLVSOnGetSessionIdInnerListener;
import com.plv.livescenes.streamer.listener.IPLVSStreamerOnLiveStreamingStartListener;
import com.plv.livescenes.streamer.listener.IPLVSStreamerOnLiveTimingListener;
import com.plv.livescenes.streamer.listener.IPLVSStreamerOnServerTimeoutDueToNetBrokenListener;
import com.plv.livescenes.streamer.listener.PLVSStreamerEventListener;
import com.plv.livescenes.streamer.listener.PLVSStreamerListener;
import com.plv.livescenes.streamer.manager.PLVSStreamerManagerFactory;
import com.plv.livescenes.streamer.mix.PLVRTCMixUser;
import com.plv.livescenes.streamer.transfer.PLVSStreamerInnerDataTransfer;
...
public class PLVStreamerPresenter implements IPLVStreamerContract.IStreamerPresenter {
    ...
    private void requestListUsersApi() {
        ...
        listUsersDisposable = PLVChatApiRequestHelper.getListUsers(roomId, memberPage, memberLength)
        ...
        listUserTimerDisposable = Observable.interval(INTERVAL_TO_GET_USER_LIST, INTERVAL_TO_GET_USER_LIST, TimeUnit.MILLISECONDS, Schedulers.io())
                .flatMap(new Function<Long, Observable<PLVSListUsersVO>>() {
                    ...
                    public Observable<PLVSListUsersVO> apply(Long aLong) throws Exception {
                        return PLVChatApiRequestHelper.getListUsers(roomId, memberPage, memberLength).retry(1);
                    }
                })
    ...
    private void generateMemberListWithListUsers(List<PLVSocketUserBean> socketUserBeanList, boolean isResetJoiningStatus) {
        ...
        Runnable addTeacherTask = new Runnable() {
            ...
            public void run() {
                ...
                memberItemDataBean.setSocketUserBean(PLVSocketWrapper.getInstance().getLoginVO().createSocketUserBean());
