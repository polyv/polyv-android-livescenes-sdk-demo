...
public class PLVMultiRoleEventProcessor {
    ...
    private void acceptJoinDiscussEvent(final PLVJoinDiscussEvent joinDiscussEvent) {
        ...
        groupLeaderId = null;
        PLVSocketWrapper.getInstance().emit(PLVEventConstant.Seminar.SEMINAR_EVENT, PLVGsonUtil.toJson(new PLVJoinDiscussEvent()), new IPLVLinkMicEventSender.PLVMainCallAck() {
            @Override
    ...
    private void acceptLeaveDiscussEvent(PLVLeaveDiscussEvent leaveDiscussEvent) {
        groupId = null;
        PLVSocketWrapper.getInstance().emit(PLVEventConstant.Seminar.SEMINAR_EVENT, PLVGsonUtil.toJson(new PLVLeaveDiscussEvent()), new IPLVLinkMicEventSender.PLVMainCallAck() {
            @Override
            public void onCall(Object... args) {
                if (args != null && args.length != 0 && args[0] != null) {
