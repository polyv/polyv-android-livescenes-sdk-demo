package com.plv.livecommon.module.modules.chatroom.presenter;
...
import com.plv.livecommon.module.data.PLVLiveRoomDataRequester;
...
import com.plv.livecommon.module.modules.chatroom.PLVSpecialTypeTag;
...
import com.plv.livecommon.module.modules.socket.PLVSocketMessage;
...
public class PLVChatroomPresenter implements IPLVChatroomContract.IChatroomPresenter {
    ...
    //聊天室功能开关数据观察者
    private Observer<PLVStatefulData<PLVChatFunctionSwitchVO>> functionSwitchObserver;
    //直播详情数据观察者
    private Observer<PLVStatefulData<PLVLiveClassDetailVO>> classDetailVOObserver;
    // </editor-fold>
    ...
    public void init() {
        ...
        //添加严禁词监听器
        PLVChatroomManager.getInstance().setProhibitedWordListener(new IPLVProhibitedWordListener() {
            @Override
        ...
        //添加在线人数监听器
        PLVChatroomManager.getInstance().setOnlineCountListener(new IPLVOnlineCountListener() {
            @Override
    ...
    @Override
    public Pair<Boolean, Integer> sendChatMessage(final PLVLocalMessage textMessage) {
        int sendValue = PLVChatroomManager.getInstance().sendChatMessage(textMessage, liveRoomDataManager.getSessionId(), true, new Ack() {
            @Override
        ...
        if (sendValue == PLVLocalMessage.SENDVALUE_BANIP) {//被禁言也认为发送成功，但不会广播给其他用户
        ...
        return new Pair<>(sendValue > 0 || sendValue == PLVLocalMessage.SENDVALUE_BANIP, sendValue);
    }


    @Override
    public Pair<Boolean, Integer> sendQuoteMessage(final PLVLocalMessage textMessage, String quoteId) {
        int sendValue = PLVChatroomManager.getInstance().sendQuoteMessage(textMessage, liveRoomDataManager.getSessionId(), true, new Ack() {
            @Override
            public void call(Object... args) {
    ...
    @Override
    public int sendQuestionMessage(final PLVQuestionMessage questionMessage) {
        int sendValue = PLVChatroomManager.getInstance().sendQuestionMessage(questionMessage);
        if (sendValue > 0) {
            //把带表情的信息解析保存下来
    ...
    public void sendLikeMessage() {
        ...
        PLVChatroomManager.getInstance().sendLikes(liveRoomDataManager.getSessionId());
        likesCount++;
        chatroomData.postLikesCountData(likesCount);
    }

    @Override
    public void sendChatImage(final PLVSendLocalImgEvent localImgEvent) {
        PLVCommonLog.d(TAG, "chatroom sendChatImage: " + localImgEvent.getImageFilePath() + ", sessionId: " + liveRoomDataManager.getSessionId());
        PLVChatroomManager.getInstance().sendChatImage(localImgEvent, liveRoomDataManager.getSessionId());
        callbackToView(new ViewRunnable() {
            @Override
            public void run(@NonNull IPLVChatroomContract.IChatroomView view) {
    ...
    public <DataBean> void sendCustomMsg(PLVBaseCustomEvent<DataBean> baseCustomEvent) {
        ...
        PLVChatroomManager.getInstance().sendCustomMsg(baseCustomEvent);
    }

    @Override
    public PLVCustomEvent<PLVCustomGiftBean> sendCustomGiftMessage(PLVCustomGiftBean customGiftBean, String tip) {
        PLVCustomEvent<PLVCustomGiftBean> customEvent = new PLVCustomEvent<>(PLVCustomGiftBean.EVENT/*自定义信息事件名*/, customGiftBean);
        customEvent.setTip(tip);
        customEvent.setEmitMode(PLVBaseCustomEvent.EMITMODE_ALL);//设置广播方式，EMITMODE_ALL为广播给包括自己的所有用户，EMITMODE_OTHERS为广播给不包括自己的所有用户
        customEvent.setVersion(PLVCustomEvent.VERSION_1);//设置信息的版本号，对该版本号的信息才进行处理
        PLVCommonLog.d(TAG, "chatroom sendCustomGiftMessage: " + customEvent);
        PLVChatroomManager.getInstance().sendCustomMsg(customEvent);
        return customEvent;
    }
    ...
    public void requestChatHistory(final int viewIndex) {
        if (PLVSocketWrapper.getInstance().isAllowChildRoom()) {
            if (!PLVSocketWrapper.getInstance().canGetChildRoomIdStatus()) {
        ...
        chatHistoryDisposable = PLVApiManager.getApichatApi().getChatHistory(loginRoomId, start, end, 1)
                ...
                .subscribe(new Consumer<List<PLVBaseViewData<PLVBaseEvent>>>() {
                    ...
                    public void accept(final Throwable throwable) throws Exception {
                        ...
                        //发送错误日志，便于排查问题
                        PLVELogSender.send(PLVChatroomELog.class, PLVChatroomELog.Event.LOAD_HISTORY_FAIL, throwable);
                        callbackToView(new ViewRunnable() {
                            @Override
                            public void run(@NonNull IPLVChatroomContract.IChatroomView view) {
    ...
    public void destroy() {
        ...
        PLVChatroomManager.getInstance().destroy();//销毁，会移除实例及所有的监听器
    }
    // </editor-fold>
            ...
            if (jsonObject != null) {
                ...
                if (!TextUtils.isEmpty(messageSource)) {
                    ...
                    if (PLVHistoryConstant.MSGSOURCE_CHATIMG.equals(messageSource)) {
                        ...
                        //如果是当前用户，则使用当前用户的昵称
                        if (PLVSocketWrapper.getInstance().getLoginVO().getUserId().equals(chatImgHistory.getUser().getUserId())) {
                        ...
                        boolean isSpecialTypeOrMe = com.plv.socket.event.PLVEventHelper.isSpecialType(chatImgHistory.getUser().getUserType())
                                || PLVSocketWrapper.getInstance().getLoginVO().getUserId().equals(chatImgHistory.getUser().getUserId());
                        ...
                        //打赏信息
                        PLVRewardEvent.ContentBean rewardContentsBean = PLVEventHelper.gson.fromJson(jsonObject_content.toString(), PLVRewardEvent.ContentBean.class);
                ...
                if (jsonObject_user != null) {
                    ...
                    //如果是当前用户，则使用当前用户的昵称
                    if (PLVSocketWrapper.getInstance().getLoginVO().getUserId().equals(speakHistory.getUser().getUserId())) {
                    ...
                    boolean isSpecialTypeOrMe = com.plv.socket.event.PLVEventHelper.isSpecialType(speakHistory.getUser().getUserType())
                            || PLVSocketWrapper.getInstance().getLoginVO().getUserId().equals(speakHistory.getUser().getUserId());
        ...
        messageDisposable = PLVRxBus.get().toObservable(PLVSocketMessage.class)
                ...
                .map(new Function<List<PLVSocketMessage>, List<PLVSocketMessage>>() {
                    ...
                    public List<PLVSocketMessage> apply(List<PLVSocketMessage> chatroomMessages) throws Exception {
                        //主线程调用PLVSocketWrapper.getInstance()
                        if (PLVSocketWrapper.getInstance().getLoginVO() == null) {
                            return new ArrayList<>();
                        }
                        return getConfig().getChannelId().equals(PLVSocketWrapper.getInstance().getLoginVO().getChannelId()) ? chatroomMessages : null;
                    }
                })
            ...
            if (PLVEventConstant.SE_CUSTOMMESSAGE.equals(listenEvent)) {
                ...
                switch (event) {
                    ...
                    case PLVCustomGiftBean.EVENT:
                        Type giftType = new TypeToken<PLVCustomEvent<PLVCustomGiftBean>>() {
                        }.getType();
                        final PLVCustomEvent<PLVCustomGiftBean> customGiftEvent = PLVEventHelper.gson.fromJson(message, giftType);
                        if (customGiftEvent != null && PLVCustomEvent.VERSION_1 == customGiftEvent.getVersion()
                                && customGiftEvent.getData() != null && customGiftEvent.getUser() != null) {
                            if (PLVSocketWrapper.getInstance().getLoginVO().getUserId().equals(customGiftEvent.getUser().getUserId())) {
                                //自己的送礼信息
                            } else {
                                //其他用户的送礼信息
                                callbackToView(new ViewRunnable() {
                ...
                switch (event) {
                    ...
                    case PLVEventConstant.Chatroom.MESSAGE_EVENT_SPEAK:
                        final PLVSpeakEvent speakEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVSpeakEvent.class);
                        if (speakEvent != null && speakEvent.getUser() != null) {
                            if (!PLVSocketWrapper.getInstance().getLoginVO().getUserId().equals(speakEvent.getUser().getUserId())) {
                                ...
                                isSpecialType = com.plv.socket.event.PLVEventHelper.isSpecialType(speakEvent.getUser().getUserType());
                                callbackToView(new ViewRunnable() {
                                    @Override
                                    public void run(IPLVChatroomContract.IChatroomView view) {
                    ...
                    case PLVEventConstant.Chatroom.MESSAGE_EVENT_CHAT_IMG:
                        final PLVChatImgEvent chatImgEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVChatImgEvent.class);
                        if (chatImgEvent != null &&
                            ...
                            isSpecialType = com.plv.socket.event.PLVEventHelper.isSpecialType(chatImgEvent.getUser().getUserType());
                            callbackToView(new ViewRunnable() {
                                @Override
                                public void run(IPLVChatroomContract.IChatroomView view) {
                    ...
                    case PLVEventConstant.Chatroom.MESSAGE_EVENT_LIKES:
                        final PLVLikesEvent likesEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVLikesEvent.class);
                        if (likesEvent != null &&
                                !PLVSocketWrapper.getInstance().getLoginVO().getUserId().
                    ...
                    case PLVEventConstant.MESSAGE_EVENT_LOGIN:
                        final PLVLoginEvent loginEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVLoginEvent.class);
                        if (loginEvent != null) {
                            ...
                            if (!PLVSocketWrapper.getInstance().getLoginVO().getUserId().equals(loginEvent.getUser().getUserId())) {
                                viewerCount++;
                    ...
                    case PLVEventConstant.MESSAGE_EVENT_LOGOUT:
                        final PLVLogoutEvent logoutEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVLogoutEvent.class);
                        if (logoutEvent != null) {
                            callbackToView(new ViewRunnable() {
                                @Override
                                public void run(IPLVChatroomContract.IChatroomView view) {
                    ...
                    case PLVEventConstant.Interact.BULLETIN_SHOW:
                        final PLVBulletinVO bulletinVO = PLVGsonUtil.fromJson(PLVBulletinVO.class, message);
                        if (bulletinVO != null) {
                    ...
                    case PLVEventConstant.Chatroom.EVENT_PRODUCT_MESSAGE:
                        ...
                        if (productEvent != null) {
                            if (productEvent.isProductControlEvent()) { //商品上架/新增/编辑/推送事件
                                final PLVProductControlEvent productControlEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVProductControlEvent.class);
                                ...
                                final PLVProductRemoveEvent productRemoveEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVProductRemoveEvent.class);
                                ...
                                final PLVProductMoveEvent productMoveEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVProductMoveEvent.class);
                                ...
                                final PLVProductMenuSwitchEvent productMenuSwitchEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVProductMenuSwitchEvent.class);
                    ...
                    case PLVEventConstant.Chatroom.MESSAGE_EVENT_CLOSEROOM:
                        final PLVCloseRoomEvent closeRoomEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVCloseRoomEvent.class);
                    ...
                    case PLVEventConstant.Chatroom.EVENT_T_ANSWER:
                        final PLVTAnswerEvent tAnswerEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVTAnswerEvent.class);
                        //判断是否是回复自己的
                        if (tAnswerEvent != null &&
                                PLVSocketWrapper.getInstance().getLoginVO().getUserId().equals(tAnswerEvent.getS_userId())) {
                            //把带表情的信息解析保存下来
                    ...
                    case PLVEventConstant.Chatroom.EVENT_REWARD:
                        final PLVRewardEvent rewardEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVRewardEvent.class);
                        if (rewardEvent != null) {
                    ...
                    case PLVRemoveContentEvent.EVENT:
                        final PLVRemoveContentEvent removeContentEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVRemoveContentEvent.class);
                    ...
                    case PLVRemoveHistoryEvent.EVENT:
                        PLVRemoveHistoryEvent removeHistoryEvent = com.plv.socket.event.PLVEventHelper.toMessageEventModel(message, PLVRemoveHistoryEvent.class);
    ...
    private void acceptLocalChatMessage(final PLVLocalMessage textMessage, String messageId) {
        //信息发送成功后，保存信息id
    ...
    private void observeLiveRoomData() {
        //观察直播间的聊天室开关数据
        functionSwitchObserver = new Observer<PLVStatefulData<PLVChatFunctionSwitchVO>>() {
            @Override
        ...
        //观察直播间的直播详情数据
        classDetailVOObserver = new Observer<PLVStatefulData<PLVLiveClassDetailVO>>() {
            @Override
            public void onChanged(@Nullable PLVStatefulData<PLVLiveClassDetailVO> classDetailVOStateData) {
                liveRoomDataManager.getClassDetailVO().removeObserver(this);
                if (classDetailVOStateData == null || !classDetailVOStateData.isSuccess()) {
                    return;
                }
                PLVLiveClassDetailVO classDetailVO = classDetailVOStateData.getData();
