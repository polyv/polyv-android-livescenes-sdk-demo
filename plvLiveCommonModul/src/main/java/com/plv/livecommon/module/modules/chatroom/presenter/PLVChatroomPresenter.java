...
public class PLVChatroomPresenter implements IPLVChatroomContract.IChatroomPresenter {
    ...
    public void init() {
        ...
        //添加房间开启关闭监听器
        PLVChatroomManager.getInstance().addOnRoomStatusListener(new IPLVChatroomManager.RoomStatusListener() {
            @Override
    ...
    public Pair<Boolean, Integer> sendChatEmotionImage(final PLVChatEmotionEvent emotionEvent) {
        ...
        int sendValue = PLVChatroomManager.getInstance().sendEmotionImage(emotionEvent, new Ack() {
            @Override
            public void call(Object... args) {
                PLVCommonLog.d(TAG, "chatroom sendTextMessage call: " + Arrays.toString(args));
                if (args == null || args.length == 0 || args[0] == null) {
    ...
    public void requestChatHistory(final int viewIndex) {
        ...
        chatHistoryDisposable = PLVApiManager.getPlvApichatApi().getChatHistory(getRoomIdCombineDiscuss(), start, end, 1, 1)
        ...
        kickUsersDisposable = PLVChatApiRequestHelper.getKickUsers(loginRoomId)
                .subscribe(new Consumer<PLVKickUsersVO>() {
                    ...
                    public void accept(Throwable throwable) throws Exception {
                        PLVCommonLog.exception(throwable);
                        //发送错误日志，便于排查问题
                        PLVELogSender.send(PLVChatroomELog.class, PLVChatroomELog.Event.GET_KICKUSERS_FAIL, throwable);
                    }
                });
    }
    ...
    public void getChatEmotionImages() {
        ...
        chatEmotionImagesDisposable = PLVApiManager.getPlvApichatApi().getEmotionImages(channel, accountId, page, size)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Consumer<PLVResponseApiBean2<PLVEmotionImageVO>>() {
                    @Override
                    public void accept(PLVResponseApiBean2<PLVEmotionImageVO> emotionImageVO) throws Exception {
                        if (emotionImageVO != null && emotionImageVO.getData() != null && emotionImageVO.getData().getList() != null) {
                            List<PLVEmotionImageVO.EmotionImage> emotionImages = emotionImageVO.getData().getList();
    ...
    public void toggleRoom(boolean isClose, IPLVChatroomManager.RequestApiListener<String> listener) {
        PLVChatroomManager.getInstance().toggleRoom(isClose, listener);
    }
