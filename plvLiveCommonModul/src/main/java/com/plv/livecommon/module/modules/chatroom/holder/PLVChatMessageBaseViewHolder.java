package com.plv.livecommon.module.modules.chatroom.holder;

import android.view.View;
import android.view.ViewGroup;

import com.plv.business.model.chat.PLVAnswerVO;
...
import com.plv.livecommon.ui.widget.itemview.adapter.PLVBaseAdapter;
...
public class PLVChatMessageBaseViewHolder<Data extends PLVBaseViewData, Adapter extends PLVBaseAdapter>
    ...
    public void processData(Data data, int position) {
        ...
        if (messageData instanceof PLVSpeakEvent) {//接收的发言事件信息
        ...
        } else if (messageData instanceof PLVLocalMessage) {//本地的发言事件信息
            fillFieldFromLoginVO(PLVSocketWrapper.getInstance().getLoginVO());
            int validIndex = Math.min(((PLVLocalMessage) messageData).getObjects().length - 1, msgIndex);
            speakMsg = (CharSequence) ((PLVLocalMessage) messageData).getObjects()[validIndex];
            prohibitedWordVO = ((PLVLocalMessage) messageData).getProhibitedWord();
            chatQuoteVO = ((PLVLocalMessage) messageData).getQuote();
            if (chatQuoteVO != null && chatQuoteVO.isSpeakMessage()) {
                quoteSpeakMsg = (CharSequence) chatQuoteVO.getObjects()[validIndex];
            }
        ...
        } else if (messageData instanceof PLVSendLocalImgEvent) {//本地的发送图片事件信息
            isLocalChatImg = true;
            PLVSendLocalImgEvent localImgEvent = (PLVSendLocalImgEvent) messageData;
            fillFieldFromLoginVO(PLVSocketWrapper.getInstance().getLoginVO());
        ...
        } else if (messageData instanceof PLVQuestionMessage) {//本地的提问信息
            fillFieldFromLoginVO(PLVSocketWrapper.getInstance().getLoginVO());
            int validIndex = Math.min(((PLVQuestionMessage) messageData).getObjects().length - 1, msgIndex);
            speakMsg = (CharSequence) ((PLVQuestionMessage) messageData).getObjects()[validIndex];
            ...
            if(tAnswerEvent.getMsgType()!=null && tAnswerEvent.getMsgType().equalsIgnoreCase(PLVTAnswerEvent.TYPE_IMAGE)){
                ...
                PLVAnswerVO imgAnswerEvent = PLVGsonUtil.fromJson(PLVAnswerVO.class,msg);
