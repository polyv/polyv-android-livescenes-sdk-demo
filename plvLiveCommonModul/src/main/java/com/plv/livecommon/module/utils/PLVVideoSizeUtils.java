package com.plv.livecommon.module.utils;
...
public class PLVVideoSizeUtils {
    ...
    //适配播放器的填充模式和位置大小
    public static void fitVideoRatioAndRect(PLVBaseVideoView baseVideoView, ViewParent viewParent, Rect rect) {
        int ratio = fitVideoRatio(baseVideoView);
        fitVideoRect(ratio == PLVPlayerScreenRatio.AR_ASPECT_FILL_PARENT, viewParent, rect);
    }
    ...
    public static int fitVideoRatio(PLVBaseVideoView baseVideoView) {
        int ratio = -1;
        if (baseVideoView != null) {
            int[] videoSize = getVideoWH(baseVideoView);
            if (videoSize[0] >= videoSize[1]) {
                ratio = PLVPlayerScreenRatio.AR_ASPECT_FIT_PARENT;
                baseVideoView.setAspectRatio(ratio);
            } else {
                ratio = PLVPlayerScreenRatio.AR_ASPECT_FILL_PARENT;
                baseVideoView.setAspectRatio(ratio);
            }
        }
        return ratio;
    ...
    //获取视频区域显示的高度
    public static int getVideoDisplayRectHeight(PLVBaseVideoView baseVideoView, ViewParent viewParent, Rect rect) {
        //视频显示区域的高度
