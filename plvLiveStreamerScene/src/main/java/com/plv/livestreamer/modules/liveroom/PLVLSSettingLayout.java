...
public class PLVLSSettingLayout extends FrameLayout implements View.OnClickListener {
    ...
    private class BitrateAdapter extends RecyclerView.Adapter<BitrateAdapter.BitrateViewHolder> {
        ...
        private String getBitrateText(int position) {
            String bitrateText = "";
            if (position == 0) {
                bitrateText = maxBitrate >= PLVStreamerConfig.Bitrate.BITRATE_SUPER ? "超清" : maxBitrate == PLVStreamerConfig.Bitrate.BITRATE_HIGH ? "高清" : "标清";
            } else if (position == 1) {
                bitrateText = maxBitrate >= PLVStreamerConfig.Bitrate.BITRATE_SUPER ? "高清" : "标清";
            } else if (position == 2) {
                bitrateText = "标清";
            }
            return bitrateText;
