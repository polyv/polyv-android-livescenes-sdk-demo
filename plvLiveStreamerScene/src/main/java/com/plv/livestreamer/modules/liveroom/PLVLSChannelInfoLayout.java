package com.plv.livestreamer.modules.liveroom;
...
import com.plv.livecommon.ui.widget.webview.PLVSafeWebView;
...
public class PLVLSChannelInfoLayout extends FrameLayout {
    ...
    private void setDescContent(PLVLiveClassDetailVO.DataBean dataBean) {
        if (dataBean == null) {
            return;
        }
    ...
    private void observeLiveRoomData() {
        ...
        liveRoomDataManager.getClassDetailVO().observe((LifecycleOwner) getContext(), new Observer<PLVStatefulData<PLVLiveClassDetailVO>>() {
            @Override
            public void onChanged(@Nullable PLVStatefulData<PLVLiveClassDetailVO> statefulData) {
                liveRoomDataManager.getClassDetailVO().removeObserver(this);
                if (statefulData == null || !statefulData.isSuccess()) {
                    return;
                }
                PLVLiveClassDetailVO liveClassDetailVO = statefulData.getData();
                if (liveClassDetailVO == null || liveClassDetailVO.getData() == null) {
                    return;
                }
                PLVLiveClassDetailVO.DataBean dataBean = liveClassDetailVO.getData();
